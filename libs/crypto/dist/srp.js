...
export class SRPAuthenticator {
    ...
    static async generateSRPCredentials(walletAddress, walletSignature) {
        try {
    ...
    static async verifySRPCredentials(walletAddress, walletSignature, storedSalt, storedVerifier) {
        try {
    ...
    static async generateSessionKey(walletAddress, walletSignature, nonce) {
        try {
    ...
    static async validateSessionKey(sessionKey, walletAddress, maxAge = 24 * 60 * 60 * 1000 // 24 hours
    ) {
        try {
...
export class SIWEChallenge {
    static generateChallenge(walletAddress) {
...
This request will not trigger a blockchain transaction or cost any gas fees.`;
    ...
    static validateChallenge(challenge, nonce, timestamp, maxAge = 5 * 60 * 1000 // 5 minutes
    ) {
        const now = Date.now();
